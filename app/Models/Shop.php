...
class Shop extends Model
{
    ...
    // {
    //     return $query->whereHas('seller', function ($query) use($userLat,$userLng) {
    //         // $query->where(['status' => 'approved']);
    //         if($userLat && $userLng && !auth('customer')->check()){
    //             $query->where(['status' => 'approved']);
    //         }else{
    //             // dd($userLat == '' , $userLng == '' , !auth('customer')->check());
    //             $query->where(['status' => 'approved'])
    //               ->whereRaw("
    //                   6371 * acos(
    ...
    public function scopeActive($query, $userLat = '', $userLng = '')
    {
            return $query->whereHas('seller', function ($query) use ($userLat, $userLng) {
                ...
                if (!auth('customer')->check()) {
                    ...
                    \Log::info("UserLat: {$userLat}, UserLng: {$userLng}");
                    $query->where(['status' => 'approved'])
                        ->whereRaw("
                            6371 * acos(
                                cos(radians(?)) * cos(radians(latitude)) *
