...
class Product extends Model
{
    ...
    public function scopeActive($query,$userLat='',$userLng='')
    {

         // \Log::info("Active Scope: {$userLat}, UserLng Api: {$userLng}");
        ...
        // if(!$userLat && ! $userLng){
        //     $userLatApi = request()->header('userLat');
        //     $userLngApi = request()->header('userLng');
        //     if($userLatApi && $userLngApi){
        //         $userLat = $userLatApi;
        //         $userLng =$userLngApi;
        //         \Log::info("UserLat Api Active Scope: {$userLat}, UserLng Api: {$userLng}");
        ...
        return $query->when($businessMode == 'single', function ($query) {
            ...
            ->SellerApproved($userLat,$userLng)
    ...
    public function scopeSellerApproved($query,$userLat='',$userLng=''): void
{

    // dump($userLat,$userLng);
    $query->whereHas('seller', function ($query) use ($userLat, $userLng) {
        $user = Helpers::getCustomerInformation(request());
      //  \Log::info($user);
        if(!auth('customer')->check() && !$user){
            ...
            //     }
            // }
            // \Log::info("Before USe Api: {$userLat}, UserLng Api: {$userLng}");
