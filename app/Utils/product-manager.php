...
class ProductManager
{
    ...
    public static function get_latest_products($request, $limit = 10, $offset = 1)
    {
        ...
        $paginator = Product::active($userLat,$userLng)
    ...
    public static function getNewArrivalProducts($request, $limit = 10, $offset = 1)
    {
        ...
        $userLng = session()->get('userLng');
        $user = Helpers::getCustomerInformation($request);
        $products = Product::active($userLat,$userLng)
            ->with(['rating', 'tags', 'seller.shop', 'flashDealProducts.flashDeal'])
            ->withCount(['reviews', 'wishList' => function ($query) use ($user) {
                $query->where('customer_id', $user != 'offline' ? $user->id : '0');
            }]);
    ...
    public static function getTopRatedProducts($request, $limit = 10, $offset = 1)
    {
        ...
        $productListData = Product::active($userLat,$userLng)->withSum('orderDetails', 'qty', function ($query) {
    ...
    public static function get_related_products($product_id, $request = null)
    {
        ...
        $products = Product::active($userLat,$userLng)->with(['rating', 'flashDealProducts.flashDeal', 'tags', 'seller.shop'])
            ->withCount(['reviews', 'wishList' => function ($query) use ($user) {
                $query->where('customer_id', $user != 'offline' ? $user->id : '0');
            }])
    ...
    public static function search_products($request, $name, $category = 'all', $limit = 10, $offset = 1): array
    {
        ...
        $productListData = Product::active($userLat,$userLng)->with(['rating', 'tags'])
    ...
    public static function getSearchProductsForWeb($name, $category = 'all', $limit = 10, $offset = 1): array
    {
        ...
        $productListData = Product::active($userLat,$userLng)->with(['rating', 'tags'])->where(function ($q) use ($name) {
    ...
    public static function get_seller_products($seller_id, $request)
    {
        ...
        $productIdsForUnknownPublisher = Product::active($userLat,$userLng)->where(['product_type' => 'digital'])->whereNotIn('id', $productIdsForPublisher)->pluck('id')->toArray();
        ...
        $productIdsForUnknownAuthor = Product::active($userLat,$userLng)->where(['product_type' => 'digital'])->whereNotIn('id', $productIdsForAuthor)->pluck('id')->toArray();
        ...
        $products = Product::active($userLat,$userLng)
            ...
            ->when($request->has('publishing_houses') && $publishingHouses, function ($query) use ($request, $publishingHouses, $productIdsForPublisher,$userLat,$userLng) {
                $publishingHouseList = PublishingHouse::whereIn('id', $publishingHouses)->with(['publishingHouseProducts'])->withCount(['publishingHouseProducts' => function ($query) use($userLat,$userLng) {
                    return $query->whereHas('product', function ($query)use($userLat,$userLng) {
                        return $query->active($userLat,$userLng);
                    });
                }])->get();
    ...
    public static function get_user_total_product($added_by, $user_id)
    {
        ...
        $total_product = Product::active($userLat,$userLng)->where(['added_by' => $added_by, 'user_id' => $user_id])->count();
    ...
    public static function getProductsColorsArray($productIds = []): array
    {
        ...
        $colorsMerge = [];
        $colorsCollection = Product::active($userLat,$userLng)->where('colors', '!=', '[]')
    ...
    public static function getPriorityWiseFlashDealsProductsQuery($id = null, $userId = null): array
    {
        ...
        if ($flashDeal) {
            $flashDealProducts = ProductManager::getPriorityWiseFlashDealsProductsQuerySorting(
                query: Product::active($userLat,$userLng)->with(['compareList']),
                flashDeal: $flashDeal,
                userId: $userId,
            );
        }
    ...
    public static function getPublishingHouseList($productIds = [], $vendorId = null): mixed
    {
        ...
        $publishingHouseList = PublishingHouse::with(['publishingHouseProducts.product'])
            ->withCount(['publishingHouseProducts' => function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
                return $query->whereHas('product', function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
                    return $query->active($userLat,$userLng)->where('product_type', 'digital')->when(!empty($productIds), function ($query) use ($productIds) {
                        return $query->whereIn('id', $productIds);
            ...
            }])->when(!empty($productIds), function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
                return $query->whereHas('publishingHouseProducts.product', function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
                    return $query->active($userLat,$userLng)->where('product_type', 'digital')->when(!empty($productIds), function ($query) use ($productIds, $vendorId) {
                        return $query->whereIn('id', $productIds);
                    })->when($vendorId && $vendorId == 0, function ($query) use ($vendorId) {
                        return $query->where(['added_by' => 'admin']);
                    })->when($vendorId && $vendorId != 0, function ($query) use ($vendorId) {
        ...
        $productCount = Product::active($userLat,$userLng)
    ...
    public static function getProductAuthorList($productIds = [], $vendorId = null): mixed
    {
        ...
        $authorList = Author::withCount(['digitalProductAuthor' => function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
            return $query->whereHas('product', function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
                return $query->active($userLat,$userLng)->where('product_type', 'digital')->when(!empty($productIds), function ($query) use ($productIds, $vendorId) {
                    return $query->whereIn('id', $productIds);
                })
            ...
            return $query->whereHas('digitalProductAuthor.product', function ($query) use ($productIds, $vendorId,$userLat,$userLng) {
                return $query->active($userLat,$userLng)->where('product_type', 'digital')->when(!empty($productIds), function ($query) use ($productIds) {
                    return $query->whereIn('id', $productIds);
                })
                    ->when($vendorId && $vendorId == 0, function ($query) use ($vendorId) {
    ...
    { 
        ...
        $publishingHouseList = PublishingHouse::with(['publishingHouseProducts'])
            ->whereHas('publishingHouseProducts.product', function ($query) use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
            })
            ->withCount(['publishingHouseProducts' => function ($query) use($userLat,$userLng) {
                return $query->whereHas('product', function ($query) use($userLat,$userLng) {
                    return $query->active($userLat,$userLng);
                });
            }])->get();
        ...
        $authorList = Author::withCount(['digitalProductAuthor' => function ($query)use($userLat,$userLng) {
            return $query->whereHas('product', function ($query)use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
            });
        }])->get();
        ...
        $productListData = Product::active($userLat,$userLng)
            ->with(['category', 'reviews', 'rating', 'seller.shop'])
            ...
            ->when($request->has('publishing_house_id') && $request['publishing_house_id'] != '' && $request['publishing_house_id'] != 0, function ($query) use ($request,$userLat,$userLng) {
                $digitalPublishingHouseIds = DigitalProductPublishingHouse::whereHas('product', function ($query) use($userLat,$userLng) {
                    return $query->active($userLat,$userLng);
            ...
            ->when($request->has('publishing_house_ids') && !empty($request['publishing_house_ids']), function ($query) use ($request, $productIdsForUnknownPublisher,$userLat,$userLng) {
                $publishingHouseList = PublishingHouse::whereIn('id', $request['publishing_house_ids'])->with(['publishingHouseProducts'])->withCount(['publishingHouseProducts' => function ($query,$userLat,$userLng) {
                    return $query->whereHas('product', function ($query) use($userLat,$userLng) {
                        return $query->active($userLat,$userLng);
                    });
                }])->get();
            ...
            ->when($request->has('author_id') && $request['author_id'] != '' && $request['author_id'] == 0, function ($query) use ($request,$userLat,$userLng) {
            ...
            ->when($request->has('author_ids') && !empty($request['author_ids']) && is_array($request['author_ids']), function ($query) use ($request, $productIdsForUnknownAuthor,$userLat,$userLng) {

                $authorList = Author::whereIn('id', $request['author_ids'])->withCount(['digitalProductAuthor' => function ($query) use($userLat,$userLng) {
                    return $query->whereHas('product', function ($query)use($userLat,$userLng) {
                        return $query->active($userLat,$userLng);
                    });
                }])->get();
    ...
    public static function getAllProductsData($request, $productUserID = null, $productAddedBy = null): mixed
    {
        ...
        return Product::active($userLat,$userLng)->with('rating')->withCount('reviews')
            ->when($productAddedBy == 'admin', callback: function ($query) use ($productAddedBy) {
                return $query->where(['added_by' => $productAddedBy]);
            })
            ->when($productUserID && $productAddedBy == 'seller', function ($query) use ($productUserID, $productAddedBy) {
                return $query->where(['added_by' => $productAddedBy, 'user_id' => $productUserID]);
