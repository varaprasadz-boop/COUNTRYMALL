...
class CartManager
{
    ...
    public static function get_cart($groupId = null, $type = null)
    {
        ...
        return Cart::with(['product' => function ($query) use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
            }])
            ->whereHas('product', function ($query) use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
            })
    ...
    public static function get_cart_group_ids($request = null, $type = null)
    {
        ...
        return Cart::whereHas('product', function ($query) use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
    ...
    public static function get_shipping_cost($groupId = null, $type = null)
    {
        ...
        $orderWiseShippingCostData = CartShipping::whereHas('cart', function ($query) use ($type,$userLat,$userLng) {
                return $query->where(['product_type' => 'physical'])->whereHas('product', function ($query) use($userLat,$userLng) {
                    return $query->active($userLat,$userLng);
                })->when($type == 'checked', function ($query) {
                    return $query->where(['is_checked' => 1]);
                });
        ...
        if (auth('customer')->check()) {
            ...
            if (count($cartGroupIds) > 0) {
                ...
                foreach ($cartGroupIds as $cart) {
                    $cartData = Cart::whereHas('product', function ($query) use($userLat,$userLng) {
                            return $query->active($userLat,$userLng);
