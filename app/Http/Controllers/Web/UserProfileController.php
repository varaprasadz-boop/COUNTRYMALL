...
class UserProfileController extends Controller
{
    ...
    public function account_address_make_primary(Request $request){
      
        
        ShippingAddress::where('customer_id', auth('customer')->user()->id)->update(['is_primary' => 0]);
        ShippingAddress::where('id', $request->post('address_id'))->update(['is_primary' => 1]);
        $address = ShippingAddress::where('id', $request->post('address_id'))->first();
        session()->put('userLat',$address['latitude']);
        session()->put('userLng',$address['longitude']);


        \Log::info('User Lat: ' . session()->get('userLat'));
        \Log::info('User Lng: ' . session()->get('userLng'));
        Artisan::call('cache:clear');

        
        Toastr::success(translate('address_updated_successfully!'));

        if($request->post('header_id')){
            return redirect()->back();
    ...
    public function address_store(Request $request): RedirectResponse
    {
        ...
        $addressId = DB::table('shipping_addresses')->insertGetId($address);

        // Check if the user has only one address
        $customerId = $address['customer_id'];
        if ($customerId) {
            $addressCount = DB::table('shipping_addresses')
