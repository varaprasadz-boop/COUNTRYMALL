...
class WebController extends Controller
{
    ...
    public function getAllCategoriesView(Request $request): View|RedirectResponse
    {
        ...
        $categories = Category::with(['product' => function ($query) use($userLat,$userLng) {
            return $query->active($userLat,$userLng)->withCount(['orderDetails']);
        }])->when($request['search'], function ($query) use ($request) {
            return $query->where('name', 'like', "%{$request['search']}%");
        })->withCount(['product' => function ($query) use($userLat,$userLng) {
    ...
    function getPriorityWiseBrandProductsQuery($request, $query)
{
    ...
    if (empty($request['order_by']) && $brandProductSortBy && ($brandProductSortBy['custom_sorting_status'] == 1)) {
        ...
        return $query->get(); // Return all results without pagination
    } else {
        return $query->orderBy('name', $orderBy)->latest()->get(); // Return all results without pagination
    }
}
    ...
    public function getAllVendorsView(Request $request): View|RedirectResponse
    {
        ...
        $vendorsList = Shop::active($userLat,$userLng)
        ...
        // $inhouseProducts = Product::active($userLat,$userLng)->with(['reviews', 'rating'])->withCount('reviews')->where(['added_by' => 'admin'])->get();
    ...
    public function getSearchedProducts(Request $request): JsonResponse
    {
        ...
        if (strpos($request['name'], $companyName) !== false) {
            $ids = Product::active($userLat,$userLng)->Where('added_by', 'admin')->pluck('id');
            array_push($product_ids, ...$ids);
        }

        $seller_products = Product::active($userLat,$userLng)->withCount('reviews')->whereIn('id', $product_ids)
    ...
    public function getCashOnDeliveryCheckoutComplete(Request $request): View|RedirectResponse|JsonResponse
    {
        ...
        $carts = Cart::whereHas('product', function ($query) use($userLat,$userLng) {
    ...
    public function seller_shop_product(Request $request, $id): View|JsonResponse
    {
        $userLat = session()->get('userLat');
        $userLng = session()->get('userLng');
        $products = Product::active($userLat,$userLng)->withCount('reviews')->with('shop')->where(['added_by' => 'seller'])
            ->where('user_id', $id)
    ...
    public function getQuickView(Request $request): JsonResponse
    {
        ...
        $more_product_from_seller = Product::active($userLat,$userLng)->withCount('reviews')->where('added_by', $product->added_by)->where('id', '!=', $product->id)->where('user_id', $product->user_id)->latest()->take(5)->get();
    ...
    public function discounted_products(Request $request): View|JsonResponse
    {
        ...
        $productData = Product::active($userLat,$userLng)->with(['reviews'])->withCount('reviews');
        ...
        if ($request->ajax()) {
            if (auth('customer')->check()) {
                ...
                if ($wishlist) {
                    ...
                    session()->put('wish_list', Wishlist::whereHas('product', function ($query) use($userLat,$userLng) {
                        return $query->active($userLat,$userLng);
    ...
    public function deleteWishlist(Request $request): JsonResponse
    {
        ...
        $userLat = session()->get('userLat');
        $userLng = session()->get('userLng');
        session()->put('wish_list', $this->wishlist->whereHas('product', function ($query) use($userLat,$userLng) {
            return $query->active($userLat,$userLng);
    ...
    public function checkValidationForCheckoutPages(Request $request): array
    {
        ...
        foreach ($cartItemGroupIDs as $groupId) {
            $isPhysicalProductExist = false;
            $cartList = Cart::whereHas('product', function ($query) use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
            })->with(['product' => function ($query) use($userLat,$userLng) {
                return $query->active($userLat,$userLng);
    ...
    public function getShopReviewList(Request $request): JsonResponse
    {
        ...
        $getProductIds = Product::active($userLat,$userLng)
