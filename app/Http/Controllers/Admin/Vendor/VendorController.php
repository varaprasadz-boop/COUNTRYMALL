...
class VendorController extends BaseController
{
    ...
    public function getEditView(Request $request,$id): View
    {
        $vendor = $this->vendorRepo->getFirstWhere(["id"=>$id]);
        return view(Vendor::EDIT[VIEW],['vendor' => $vendor]);
    }
    ...
    public function update(VendorUpdateRequest $request,$id): JsonResponse
    {
        $this->vendorRepo->update($id, $this->vendorService->getEditData($request));
        // $vendors = $this->vendorRepo->getFirstWhere(["id"=>$id]);
        $shop = $this->shopRepo->getFirstWhere(["id"=>$id]);
        $getShopDataForUpdate = $this->shopService->getShopDataForUpdate($request, $shop);
        $getShopDataForUpdate['image'] = $getShopDataForUpdate['logo'];
        $this->shopRepo->update($shop['id'],$getShopDataForUpdate);
        // dd($shop,$this->shopService->getShopDataForUpdate($request, $shop));

        // $this->vendorWalletRepo->add($this->vendorService->getInitialWalletData(vendorId: $vendor['id']));
        // $data = [
        //     'vendorName' => $request['f_name'],
        //     'status' => 'pending',
        //     'subject' => translate('Vendor_Updation_Successfully_Completed'),
        //     'title' => translate('Vendor_Updation_Successfully_Completed'),
        //     'userType' => 'vendor',
        //     'templateName' => 'registration',
        // ];
        //event(new VendorRegistrationEvent(email: $request['email'], data: $data));
    //    dd($shop,$this->shopService->getShopDataForUpdate($request, $shop));
        ...
        return response()->json(['message' => translate('vendor_updated_successfully')]);
    }
