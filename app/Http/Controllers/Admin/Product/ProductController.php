...
use App\Exports\ProductListExportToImport;
...
class ProductController extends BaseController
{
    ...
    public function cloneProductToVendor(int $id,int $seller_id, ProductService $service): View|RedirectResponse
    {
       
        $product = $this->productRepo->getFirstWhereWithoutGlobalScope( ['id' => $id],  ['digitalVariation', 'seoInfo']);
  
        $dataArray = $service->cloneAddProductData($product,  'seller',$seller_id);
        $savedProduct = $this->productRepo->add( $dataArray);
        $this->productRepo->addRelatedTags($product, $savedProduct,1);
    
        $this->translationRepo->add($product,  'App\Models\Product',  $savedProduct->id,1);
        $this->updateProductAuthorAndPublishingHouse($product,  $savedProduct);

        // $digitalFileArray = $service->getAddProductDigitalVariationData($product,  $savedProduct,1);
        // foreach ($digitalFileArray as $digitalFile) {
        //     $this->digitalProductVariationRepo->add( $digitalFile);
        // }

        $this->productSeoRepo->add($service->getProductSEOData($product,  $savedProduct,  'add',1));
       
        Toastr::success(translate('Product_cloned_successfully'));
    ...
    public function exportListToImport(Request $request, string $type): BinaryFileResponse
    {
     
        // $vendorId = auth('seller')->id();
        // $vendor = $this->sellerRepo->getFirstWhere(params: ['id' => $vendorId]);
        ...
        $products = $this->productRepo->getListWhere( ['id' => 'desc'],  $request['searchValue'], $filters, ['translations'],'all');
        // dd($products);
        ...
        return Excel::download(new ProductListExportToImport($data), ucwords($request['type']) . '-' . 'product-list.xlsx');
    }
