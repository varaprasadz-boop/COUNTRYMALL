...
class CategoryController extends Controller
{
    ...
    // new api
    public function get_categories_within_radius(Request $request): JsonResponse
    {
        // Validate the required parameters
        $validated = $request->validate([
            'latitude' => 'required|numeric',
            'longitude' => 'required|numeric',
            'radius' => 'nullable|numeric|min:1|max:50', // Default radius is 3km, maximum is 50km
        ]);
    
        $latitude = $validated['latitude'];
        $longitude = $validated['longitude'];
        $radius = $validated['radius'] ?? 3; // Default radius of 3km
    
        // Get categories of sellers within the radius
        $categories = CategoryManager::getCategoriesWithinRadius($latitude, $longitude, $radius);
