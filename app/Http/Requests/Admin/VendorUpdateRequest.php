...
{
    $vendorId = $this->route('id'); // Adjust this key if it's named differently in the route
    return [
        'f_name' => 'required|string|max:255',
        'l_name' => 'nullable|string|max:255',
        'phone' => 'required|string|min:4|max:20|unique:sellers,phone,' . $vendorId,
        'email' => 'nullable|email|unique:sellers,email,' . $vendorId,
        'password' => $this->isMethod('post') ? 
            'required|min:8|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*\W)(?!.*\s).{8,}$/|same:confirm_password' 
            : 
            'nullable|min:8|regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*\W)(?!.*\s).{8,}$/|same:confirm_password',
        ...
        'warranty_details' => 'nullable|string|max:1000',
        ...
        'image' => 'nullable|image|mimes:jpg,jpeg,png,webp,gif,bmp,tif,tiff|max:2048',
        'logo' => 'nullable|image|mimes:jpg,jpeg,png,webp,gif,bmp,tif,tiff|max:2048',
        'banner' => 'nullable|image|mimes:jpg,jpeg,png,webp,gif,bmp,tif,tiff|max:2048',
        // 'status' => 'required|in:approved,pending',
        'business_registration_number' => 'nullable|string|max:255',
        'gst_tax_id' => 'nullable|string|max:255',
        'business_address' => 'nullable|string|max:255',
        'primary_products_services' => 'nullable|string|max:1000',
        'years_in_business' => 'nullable|integer|min:0|max:100',
        'from_submit' => 'nullable|string|in:admin',
