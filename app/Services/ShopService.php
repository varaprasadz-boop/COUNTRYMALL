...
class ShopService
{
    ...
    public function getShopDataForUpdate(object $request , object $shop):array
    {
        ...
        $logo = $request['logo'] ? $this->update(dir:'shop/', oldImage: $shop['image'], format: 'webp',image:  $request->file('logo')) : $shop['image'];
        $banner = $request->file('banner') ? $this->update(dir: 'shop/banner/',oldImage:  $shop['banner'], format: 'webp',image:  $request->file('banner')): $shop['banner'];
        ...
        return [
            'name' => $request->input('name') ?: $request->input('shop_name'),
            'address'=>$request->input('address')?:$request['shop_address'],
