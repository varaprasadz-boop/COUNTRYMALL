...
class ProductService
{
    ...
    public function getProcessedImages(object $request,int $is_clone=0): array
    {
        if($is_clone){
            return [
                'image_names' => $request['images'],
                'colored_image_names' => $request['color_image']
            ];
        }else{
            $colorImageSerial = [];
            $imageNames = [];
    ...
    public function getOptions(object $request,int $is_clone=0): array
    {
        if($is_clone){
            return json_decode($request['colors']);
        }else{
            $options = [];
            if ($request->has('colors_active') && $request->has('colors') && count($request->colors) > 0) {
    ...
    public function cloneAddProductData(object $request, string $addedBy,int $seller_id=0): array
    {
        $storage = config('filesystems.disks.default') ?? 'public';
        $processedImages = $this->getProcessedImages($request,1); //once the images are processed do not call this function again just use the variable
        $combinations = $this->getCombinations($this->getOptions($request,1));
        $variations = $this->getVariations( $request, $combinations);
        $stockCount = count($combinations[0]) > 0 ? $this->getTotalQuantity($variations) : (integer)$request['current_stock'];
        ...
        $digitalFileOptions = $this->getDigitalVariationOptions($request,1);
        $digitalFileCombinations = $this->getDigitalVariationCombinations( $digitalFileOptions,1);

        return [
            'added_by' => $addedBy,
            'user_id' => ($addedBy == 'admin') ? (auth('admin')->id()) : (($seller_id != 0)?($seller_id):(auth('seller')->id())),
            'name' => $request['name'],
            'code' => $request['code'],
            'slug' => $request['slug'],
            ...
            'digital_product_file_types' => $request['extensions_type'],
            ...
            'request_status' => 1,
            'shipping_cost' =>  0,
            'multiply_qty' => $request['multiply_qty'], //to be changed in form multiply_qty
            'color_image' => $request['color_image'],
            'images' => $request['images'],
            'thumbnail' => $request['thumbnail'],
            'thumbnail_storage_type' => $request['thumbnail_storage_type'],
            'preview_file' => $previewFile,
            'preview_file_storage_type' => $request['preview_file_storage_type'],
    ...
    public function getAddProductData(object $request, string $addedBy,int $seller_id=0): array
    {
        $storage = config('filesystems.disks.default') ?? 'public';
...
//            'purchase_price',
        ...
        foreach ($collections as $collection) {
            ...
            $products[] = [
                ...
                'slug' => Str::slug($collection['name'], '-') . '-' . Str::random(16),
    ...
    public function getAddProductDigitalVariationData(object $request, object|array $product,int $is_clone=0): array
    {
        $digitalFileOptions = $this->getDigitalVariationOptions(request: $request);
        $digitalFileCombinations = $this->getDigitalVariationCombinations(arrays: $digitalFileOptions);
    ...
    public function getDigitalVariationOptions(object $request,int $is_clone=0): array
    {
        if($is_clone){
            return $options = [];
        }else{
    ...
    public function getDigitalVariationCombinations(array $arrays,int $is_clone=0): array
    {
        if($is_clone){
            return $result = [];
        }else{
    ...
    public function getProductSEOData(object $request, object|null $product = null, string $action = null,int $is_clone=0): array
    {
        if($is_clone){
            return [
                "product_id" => $product['id'],
