...
<style>
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-8 mt-3 mt-lg-0 d-flex flex-wrap gap-3 justify-content-lg-end">
                                <div class="dropdown">
                                    ...
                                    <a type="button" class="btn btn-outline--primary text-nowrap" href="{{ route('admin.products.export-excel-to-import',['type'=>request('type')]) }}?brand_id={{request('brand_id') }}&searchValue={{ request('searchValue') }}&category_id={{request('category_id') }}&sub_category_id={{request('sub_category_id') }}&sub_sub_category_id={{request('sub_sub_category_id') }}&seller_id={{request('seller_id') }}&status={{request('status') }}">
                                        <img width="14" src="{{dynamicAsset(path: 'public/assets/back-end/img/excel.png')}}" class="excel" alt="">
                                        <span class="ps-2">{{ translate('exportToImport') }}</span>
                                    </a>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            <a href="javascript:void(0);" onclick="$(this).parent().parent().find('.clone').toggleClass('hide')" class="btn btn-outline-info btn-sm square-btn">
                                                <i class="tio-copy"></i>
                                            </a>
                                        ...
                                        <!-- <form method="get" class="hide clone cloneForm" action="{{ route('admin.products.clone',[$product['id'],1]) }}"> -->
                                            <div class="row">
                                                <div class="col-md-8">
                                                    <select class="form-control sellerSelect" name="seller_id">
                                                        @foreach($sellers as $seller)
                                                ...
                                                <div class="col-md-4">
                                                    <input type="button" value="Clone to" class="btn btn-primary cloneProductBtn" data-id="{{$product['id']}}">
                                                </div>
                                            </div>
                                        <!-- </form> -->
                                </td>
    ...
    <script>
    // When any form with class 'cloneForm' is about to be submitted, change the action dynamically
    document.querySelectorAll('.cloneForm').forEach(function(form) {
        form.addEventListener('submit', function(event) {
            var sellerId = $('.sellerSelect').val(); // Get selected seller_id from the form
            var product_id = this.querySelector('.product_id').value; // Get selected product_id from the form
            var url = "{{ route('admin.products.clone', [':product_id', ':seller_id']) }}"; // URL template
            url = url.replace(':product_id', product_id); // Replace the placeholder with the actual seller_id
            url = url.replace(':seller_id', sellerId); // Replace the placeholder with the actual seller_id
            location.href = url;
            // Update the form action with the correct URL (with seller_id in the path)
            // console.log($(this).attr('action',url));
            // this.action = url;
            $(this).attr('action',url)

            // Optionally, prevent form submission until URL is updated (though this isn't strictly necessary here)
